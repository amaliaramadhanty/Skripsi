 ...
 * @author Amalia
 */
public class CobaManual extends javax.swing.JFrame {
    ...
    int jumlahData;
    double[][] inputWeight= {{0.970588,-0.416182,-0.778919,-0.205977},{-0.293603,0.274177,0.770641,0.717264}};
    double[] bias={0.300383,-0.812157};
    double[][] outputHidden;
    double[][] outputWeight;
...
//                TableModel model = new DefaultTableModel(sheet.getRows(), sheet.getColumns());
                for (int row = 0; row < sheet.getRows(); row++) {
                    ...
                    data.addRow(new Object[]{row + 1, isiData[row][0], isiData[row][1], isiData[row][2],
                        isiData[row][3], target[row]});
                }
    ...
    void normalisasi() {
        ...
        for (int i = 0; i < jumlahData; i++) {
            ...
            normalisasi.addRow(new Object[]{i + 1, isiData[i][0], isiData[i][1], isiData[i][2],
                isiData[i][3], target[i]});
        }
    }
    ...
    void inisialisasi() {
        for (int i = 0; i < jumlahHiddenLayer; i++) {
            inputW.addRow(new Object[]{i + 1, inputWeight[i][0], inputWeight[i][1], inputWeight[i][2], inputWeight[i][3]});
            biasT.addRow(new Object[]{bias[i]});
            System.out.println(inputWeight[i][0]);
            System.out.println(inputWeight[i][1]);
            System.out.println(inputWeight[i][2]);
            System.out.println(inputWeight[i][3]);
            System.out.println("");
        }
    }
...
//            System.out.println();
    ...
    void outputLayer() {
        ...
        for (int i = 0; i < jumlahTesting; i++) {
            for (int j = 0; j < jumlahHiddenLayer; j++) {
//                output[i] = output[i] + (outputWeight[j][0] * outputHidden[i][j]);
            }
...
//        data.setRowCount(400);
    ...
    private void buttonHitungActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonHitungActionPerformed
        ...
        nilaiError();
        mse();
        jLabel10.setText("Nilai MSE: " + nilaiMse);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CobaManual.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new CobaManual().setVisible(true);
            }
        });
    }
